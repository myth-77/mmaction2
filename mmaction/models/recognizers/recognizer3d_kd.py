...
@RECOGNIZERS.register_module()
class Recognizer3Dkd_RBG2Res_Feat(BaseRecognizer):
    """3D recognizer model framework with crose model (RGB -> Res) knowledge distillation."""

    def __init__(self, backbone, teacher, teacher_path='', cls_head=None, neck=None, train_cfg=None, test_cfg=None, loss_kd=dict(type='MSELoss'), weight_loss=(1.0, 1.0)):
        super().__init__(backbone, cls_head, neck, train_cfg, test_cfg)
        self.teacher = Recognizer3D(**teacher)
        self.loss_kd = build_loss(loss_kd)
        self.weight_loss = weight_loss
        self.downsample = nn.Conv1d(2048, 768, 1)
        load_checkpoint(self.teacher, teacher_path)
    ...
    def forward_train(self, imgs, labels, **kwargs):
        ...
        with torch.no_grad():
            feat_teacher = self.teacher.extract_feat(rgb)
        feat_teacher = feat_teacher.detach().clone()
        imgs = imgs[:, :, 3:]   # residual
        imgs = imgs.reshape((-1, ) + imgs.shape[2:])
        losses = dict()

        x = self.extract_feat(imgs)
        # align feat with teacher
        x_down = nn.functional.adaptive_avg_pool3d(x, (1, 1, 1)).squeeze(-1).squeeze(-1)
        x_down = self.downsample(x_down).squeeze(-1)
        ...
        for key in loss_cls.keys():
            losses[key] = loss_cls[key] * self.weight_loss[0]
        losses['loss_kd'] = self.loss_kd(x_down, feat_teacher) * self.weight_loss[1]
        return losses
