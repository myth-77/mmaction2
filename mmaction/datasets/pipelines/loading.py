...
class DecoderCompressedVid:
    ...
    def __init__(self, format = 'mpeg4', gop_size = 12, **kwargs):
        self.format = format
        self.kwargs = kwargs
        if format == 'mpeg4':
            self.format = 4
    ...
    def _cal_gop_pos(self, frame_id):
        return frame_id // self.gop_size, frame_id % self.gop_size
    ...
    def __call__(self, results):
        ...
        for i, frame_idx in enumerate(results['frame_inds']):
            ...
            frame_idx += offset
            gop_idx, gop_pos = self._cal_gop_pos(frame_idx)
            print(f'video path:{video_path}')
            print(f'gop info :{gop_idx},{gop_pos}: {frame_idx} : {0 if gop_pos == 0 else 2}: {self.format}')
            raise SystemExit
            imgs.append(md.decode(video_path, self.format, gop_idx, gop_pos, 0 if gop_pos == 0 else 2, 0))
